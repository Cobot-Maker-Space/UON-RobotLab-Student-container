# 📘 `build-image.yml` explained, block by block — Windows branch

**What this is:** a walkthrough of `.github/workflows/build-image.yml` **as it exists on this
`windows` branch** - what it publishes, why it's built the way it is, and exactly how to run it.
Each OS branch (`windows`, `linux`, `macos`) carries its own copy of this workflow, tuned for that
OS; this file documents the `windows` branch's copy specifically. If you're comparing branches,
...
1. Works out **what image name and tag** it is about to publish:
   `ghcr.io/cobot-maker-space/robotlab-devcontainer-windows:<tag>`.
...
impossible**, and about not needing a local Windows apt-get-everything build before you can even
...
```yaml
name: Build dev container image (Windows / amd64)
```
...
| ~~`push: branches:`~~ | **Absent on purpose.** Pushing a commit to `windows` publishes nothing by itself. |
...
workflow from"** dropdown to pick `windows` - GitHub then runs *that branch's* copy of this file,
...
env:
  REGISTRY: ghcr.io
  IMAGE_BASENAME: robotlab-devcontainer-windows
```
...
- **Resolve image name and tag** (`id: meta`) - assembles `ghcr.io/cobot-maker-space/robotlab-devcontainer-windows`
...
linux (same amd64 target) and macos (arm64, needs QEMU).
...
- name: Smoke test the published image
  run: |
    ...
    docker run --rm "$IMAGE" bash -lc 'touch /home/ros2_ws/build/.write-test && rm /home/ros2_ws/build/.write-test'
```
...
## Part 4 — How to run this, specifically, on `windows`

**Manually (normal case):**

1. GitHub → **Actions** tab → **Build dev container image (Windows / amd64)** in the
   left sidebar.
2. Click **Run workflow**.
3. **Use workflow from:** the dropdown - pick **`windows`**. This is the step people get wrong;
...
4. **tag:** type the tag exactly as it appears in `windows`'s `src/.devcontainer/devcontainer.json`
   `"image"` field, e.g. `humble-sim-2026-09`. If it doesn't match, step 3.6 stops the run and
   tells you both values.
5. **push_latest:** leave checked to also move `robotlab-devcontainer-windows:latest`, or uncheck to publish
   only the dated tag.
...
at whatever's currently committed on `windows`; pushes it to
`ghcr.io/cobot-maker-space/robotlab-devcontainer-windows` under the tag you typed (and `:latest` if ticked);
...
```bash
git tag humble-sim-2026-09 windows
...
Packages tab → `robotlab-devcontainer-windows` → Package settings → Change visibility. Then verify anonymous
pull:

```bash
R=cobot-maker-space/robotlab-devcontainer-windows
...
|---|---|
| Verify devcontainer.json pins... | The tag you typed ≠ the pin in `windows`'s `devcontainer.json`, **or** you dispatched from the wrong branch in step 3 of Part 4 (e.g. `main`, which has no `"image"` field at all). The log prints both values - compare them. |
...
**"Why amd64 only?"**
Students run this container inside WSL2, which is a real amd64 Linux kernel under Docker Desktop's hood - the same architecture as the linux branch. Building the wrong architecture wouldn't fail the build - it would produce an image
...
- **It does not build any architecture other than amd64** on this branch. See linux (same amd64 target) and macos (arm64, needs QEMU)
  for the others.
